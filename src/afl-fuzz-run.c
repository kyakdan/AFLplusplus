  ...
  if (afl->custom_mutators_count) {
    ...
    LIST_FOREACH(&afl->custom_mutator_list, struct custom_mutator, {
      ...
      if (el->afl_custom_trim) {
        ...
        trimmed_case = trim_case_custom(
            afl, q, in_buf, el, vp_trim_guard, vp_trim_guard_before_exec,
            vp_trim_guard_preserved, vp_trim_guard_after_exec);
        custom_trimmed = true;
    ...
    if (custom_trimmed) {
      ...
      if (unlikely(afl->value_profile_active && had_vp_ref)) {
        ...
        if (had_vp_ref && q->vp_ref_cnt && q->len != orig_len) {

          if (unlikely(vp_trim_guard)) {

            vp_trim_guard_refresh_owner_cost(vp_trim_guard);
    ...
    while (remove_pos < q->len) {
      ...
      if (cksum == q->exec_cksum) {
        ...
        if (unlikely(vp_trim_guard)) {
          ...
          vp_ok = vp_trim_guard_preserved(vp_trim_guard, in_buf, q->len,
